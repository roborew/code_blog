...
# Define a custom nav using Tailwind classes
# Pagy::DEFAULT[:nav] = "<nav class='flex items-center justify-between border-t border-gray-200 px-4 sm:px-0 py-3 mt-6'><span class='flex-1 flex justify-between sm:hidden'>%{prev} %{next}</span><span class='hidden sm:flex-1 sm:flex sm:items-center sm:justify-between'><span class='text-sm text-gray-700'>%{info}</span>%{nav}</span></nav>"

# Pagy::DEFAULT[:nav_html] = {
#   container:    "<div class='hidden sm:flex-1 sm:flex sm:items-center sm:justify-between'>%{info} %{nav}</div>",
#   nav:          "<nav class='relative z-0 inline-flex rounded-md shadow-sm -space-x-px' aria-label='Pagination'>%{pages}</nav>",
#   info:         "<span class='text-sm text-gray-700'>Showing %{from} to %{to} of %{count} results</span>",
#   page:         "<a href='%{href}' class='bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium %{active_class}'>%{page}</a>",
#   active_page:  "<a href='%{href}' class='z-10 bg-blue-50 border-blue-500 text-blue-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium'>%{page}</a>",
#   gap:          "<span class='relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700'>...</span>",
#   disabled:     "<span class='relative inline-flex items-center px-4 py-2 border border-gray-300 bg-gray-100 text-sm font-medium text-gray-500 cursor-not-allowed'>%{text}</span>",
#   prev:         "<a href='%{href}' rel='prev' aria-label='Previous' class='relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50'>&lsaquo;</a>",
#   next:         "<a href='%{href}' rel='next' aria-label='Next' class='relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50'>&rsaquo;</a>"
# }
